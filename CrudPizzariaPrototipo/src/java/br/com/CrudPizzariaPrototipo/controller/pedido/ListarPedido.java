...
public class ListarPedido extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            PedidoImplDAO dao = new PedidoImplDAO();
            HttpSession session = request.getSession(true);

            if (session.getAttribute("cliente") != null) {
                // é um cliente listando seus pedidos
                Cliente cliente = (Cliente) session.getAttribute("cliente");
                request.setAttribute("pedidos", dao.Listar(cliente.getIdCliente()));
                request.getRequestDispatcher("pedido/listarPedidoCliente.jsp").forward(request, response);
            } else if (session.getAttribute("funcionario") != null) {
                // é um funcionario listando os pedidos dos maluco
                request.setAttribute("pedidos", dao.Listar());
                request.getRequestDispatcher("pedido/listarPedido.jsp").forward(request, response);
            } else {
                //num tá logado espero que morra.
                response.sendRedirect(request.getContextPath() + "/Clientes");
            }
        } catch (Exception e) {
            System.out.println("Problemas no Servlet ao listar pedido! Erro: " + e.getMessage());
        }
    }
