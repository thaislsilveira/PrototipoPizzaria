...
public class CadastrarEntregador extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (dao.cadastrar(oEntregador)) {
                mensagem = "Entregador cadastrado com sucesso!";
            } else {
                mensagem = "Problemas ao cadastrar Entregador. Verifique os dados informados e tente"
                        + " novamente! ";
            }
            request.setAttribute("mensagem", mensagem);
            request.getRequestDispatcher("ListarEntregador").forward(request, response);
        } catch (Exception ex) {
            System.out.println("Problemas no Servlet ao cadastrar Entregador! Erro: " + ex.getMessage());
        }
    }
