 ...
 * @author PC 05
 */
@WebServlet(name = "CadastrarFuncao", urlPatterns = {"/CadastrarFuncao"})
       ...
       response.setContentType("text/html;charset=UTF-8");
        ...
        try {
            ...
            if (dao.cadastrar(oFuncao)) {
                mensagem = "Função cadastrado com sucesso!";
            } else {
                mensagem = "Problemas ao cadastrar Função. Verifique os dados informados e tente"
                        + " novamente! ";
            }
            request.setAttribute("mensagem", mensagem);
            request.getRequestDispatcher("ListarFuncao").forward(request, response);
        } catch (Exception ex) {
            System.out.println("Problemas no Servlet ao cadastrar Função! Erro: " + ex.getMessage());
        }
    }
