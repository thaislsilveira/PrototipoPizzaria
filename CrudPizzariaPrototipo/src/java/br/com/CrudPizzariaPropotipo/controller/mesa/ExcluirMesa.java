 ...
 */
@WebServlet(name = "ExcluirMesa", urlPatterns = {"/ExcluirMesa"})
        ...
        response.setContentType("text/html;charset=UTF-8");
         try (PrintWriter out = response.getWriter()) {
            ...
            try{
                ...
                if(dao.excluir(idMesa)){
                    mensagem = "Mesa excluída com sucesso!!";
                }else{
                    mensagem = "Problemas ao excluir Mesa!!";
                }     
                request.setAttribute("mensagem", mensagem);
                request.getRequestDispatcher("ListarMesa").forward(request, response);
            }catch(Exception ex){
                System.out.println("Problemas no Servlet ao excluir Mesa!! Erro : " + ex.getMessage());
            }
        }
    }
