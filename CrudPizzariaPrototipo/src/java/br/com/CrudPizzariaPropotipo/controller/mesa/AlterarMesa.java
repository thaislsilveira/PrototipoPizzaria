 ...
 */
@WebServlet(name = "AlterarMesa", urlPatterns = {"/AlterarMesa"})
public class AlterarMesa extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        oMesa.setIdMesa(idMesa);

        try {
            GenericDAO dao = new MesaImplDAO();
            if (dao.alterar(oMesa)) {
                mensagem = "Mesa alterado com sucesso!";
                request.setAttribute("mensagem", mensagem);
                request.getRequestDispatcher("ListarMesa").forward(request, response);
            } else {
                mensagem = "Problemas ao alterado Mesa. Verifique os dados informados e tente"
                        + " novamente! ";
                request.setAttribute("mensagem", mensagem);
                request.getRequestDispatcher("mesa/cadastrarMesa.jsp").forward(request, response);
            }
        } catch (Exception ex) {
            System.out.println("Problemas no Servlet ao alterado Mesa! Erro: " + ex.getMessage());
        }
    }
